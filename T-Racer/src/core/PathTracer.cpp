...
//#ifdef TRUE
			if (triangleIndex != T_RACER_TRIANGLE_NULL) 
			{
				...
				{
					lightValue.colour = lightValue.colour + calculateDirectLighting(i, lightPath[i].pathColour).colour;
				}
...
void T_racer_Renderer_PathTracer::tracePath(T_racer_Math::Ray initialRay, T_racer_Math::Colour& irradiance)
{
	...
	//	pathTroughput = ((pathTroughput * brdfValue) * T_racer_Math::dot(wi.direction, lightPath[pathIndex].normal)) / wi.probabilityDensity;
		...
		if (!terminatePath) 
		{
			...
			triangleIndex = sceneObject->traceRay2(lightPath[pathIndex].hitPoint, wi.direction, intersectDisc);
...
T_racer_Math::Colour T_racer_Renderer_PathTracer::calculateDirectLighting(int pathVertex, T_racer_Math::Colour& col)
{
	...
	float gTerm = geometryTerm(light_wi, brdf_wi, pathVertex, lightSource);

	//assert(visible);

	T_racer_Math::Colour brdfValue = lightSource->Evaluate(lightPath[pathVertex]);  /*material->Evaluate(nullptr, lightPath[pathVertex]).getPixelValue(0,0);*/
	Ld.colour = col.colour * brdfValue.colour * visible * gTerm / light_wi.probabilityDensity;
...
{
	T_racer_TriangleIntersection intersect;
	int intersectionIndex = sceneObject->traceRay2(lightPath[pathVertex].hitPoint, (lightSource->getPosition() - lightPath[pathVertex].hitPoint).normalise(), intersect);
